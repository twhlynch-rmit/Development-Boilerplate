...
Pages: `/` · `/auth/signin` · `/auth/signup` · `/dashboard` · `/team` · `/profile` · `/settings` (route groups `(auth)`, `(dashboard)`). Backend: `GET /api/health` (public); everything else under `/api` requires `Authorization: Bearer <ID token>`.
